...
//TODO: revisit
object SortListAccToLengthOfSublist {

  def lsort[A](ls: List[List[A]]): List[List[A]] ={
//    val xs = ls map {e=> e.sortWith(_>_)}
    ls sortWith { _.length < _.length }
  }

/*
  def lsortFreq[A](ls: List[List[A]]): List[List[A]] = {
    val freqs = Map(ModifiedRunLengthEncoding.lenghtEncoding(ls map { _.length } sortWith { _<_ }) map { _.swap }:_*)
    ls sortWith { (e1, e2) => freqs(e1.length) < freqs(e2.length) }
  }*/

  def main(args: Array[String]): Unit = {

    val beforeSort = List(List('a, 'b, 'c), List('d, 'e), List('f, 'g, 'h), List('d, 'e), List('i, 'j, 'k, 'l), List('m, 'n), List('o))
    println(lsort(beforeSort))
    println(" dummy for testing by dilip"+beforeSort map (e=> e.toString) sortWith(_<_))
